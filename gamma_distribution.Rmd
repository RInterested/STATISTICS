...
Let's define a Poisson counting process with the number of arrivals at a certain time denoted as $N(t)$. As a Poisson process $N(0)=0$, the number of arrivals by time $t$ follows a Poisson distribution,
...
where $\lambda$ is the rate parameter in $\text{Pois}(\lambda t)$ with $n\in \mathbb N.$

The additional condition is that the inter-arrival times $X_1, X_2,\dots$ are iid. The time to the first arrival is $X_1$ while the time between the first and second arrival is $X_2.$ Therefore, the **inter-arrival time** between arrival $k-1$ and $k$ is $X_k,$ while the **time to the $n$-th arrival** is
...
$$\begin{align}
F_{X_1}(x) &= \Pr(X \leq x) \\
&= \Pr(\text{at least 1 arrival happens before x})\\
&=1- \Pr(\text{no arrivals by time t})\\
&=1 - \Pr(N(t)=0)\\
&= 1 - \frac{\left(\lambda t\right)^0\, e^{-{\lambda t}}}{0!}\\
&= 1 - e^{-\lambda t}
\end{align}$$

which is the CDF of the exponential distribution. So $X_1 \sim \text{Exp}(\lambda)$.

Now the distribution of $S_n$ is

$$\begin{align}
F_{S_n}(s)&=\Pr(S_n\leq s)\\
&=\Pr(n\text{ or more arrivals before time }s)\\
&=\Pr(N(s) \geq n)\\
&= \sum_{k=n}^{\infty} \frac{(\lambda s)^k\, e^{-\lambda s}}{k!}
\end{align}$$

The pdf of the time to the $s$ arrival will be the derivative
...
&= e^{-\lambda s}\left( \sum_{k=n}^{\infty}\frac{s^{k-1}\lambda^k}{(k-1)!} - \sum_{k=n}^{\infty} \frac{s^{k}\lambda^{k+1}}{k!}  \right)\\
\end{align}
$$

Now the first term in the sum

$$\sum_{k=n}^{\infty}\frac{s^{k-1}\lambda^k}{(k-1)!}=\frac{\lambda^n s^{n-1}}{(n-1)!} + \sum_{k=n+1}^{\infty}\frac{s^{k-1}\lambda^k}{(k-1)!}=\frac{\lambda^n s^{n-1}}{(n-1)!} + \sum_{k=n}^{\infty}\frac{s^{k}\lambda^{k+1}}{(k-1)!}$$
